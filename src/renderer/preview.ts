...
// ── Measurement options (used by pagination) ──────────────────────────────────
...
// Boundary rebalancing uses heuristic pass-1 pagination only.
// Advanced print-faithful optimization is not included in the public build.
async function rebalanceBoundaries(
  pagePlans: PagePlan[],
  _boundaries: BreakBoundary[],
  _contentH: number,
  _opts: MeasurementOpts,
  _pushTrace: (event: Omit<PaginationTraceEvent, 'page'> & { page?: number }) => void,
): Promise<PagePlan[]> {
  return pagePlans
}
...
export function initPreview(settings: AppSettings): void {
  ...
  window.addEventListener('app:activeFile', (e: Event) => {
    const detail = (e as CustomEvent<{ path: string | null; fileKey: string | null; content?: string }>).detail
    _currentFilePath = detail.path
    if (detail.fileKey) _currentFileKey = detail.fileKey
    // Use content from event if provided (avoids debounce race on tab switch)
    const markdown = detail.content ?? _lastMarkdown
    if (markdown) {
      _lastMarkdown = markdown
      _lastContentHash = ''
      renderAndPaginate(markdown)
    }
  })
}
...
function renderAndPaginate(markdown: string): void {
  ...
  void (async () => {
    try {
      ...
      requestAnimationFrame(() => {
        ...
        void (async () => {
          try {
            ...
            console.error('[Preview] paginate failed, falling back to single-page render:', err)
            if (gen !== _renderGen) return
            previewContent.dataset.masterHtml = htmlWithMermaid
            previewContent.innerHTML = renderPageShell({ html: htmlWithMermaid }, 0)
            window.vanfolioAPI.sendPreviewUpdate({ html: previewContent.innerHTML, fileKey: _currentFileKey })
            initTocActiveSync()
          }
        })()
      ...
      console.error('[Preview] render pipeline failed, falling back to single-page render:', err)
      if (gen !== _renderGen) return
      previewContent.dataset.masterHtml = html
      previewContent.innerHTML = renderPageShell({ html }, 0)
      window.vanfolioAPI.sendPreviewUpdate({ html: previewContent.innerHTML, fileKey: _currentFileKey })
      initTocActiveSync()
    }
  })()
